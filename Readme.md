...
![pano-sample.jpg](./Samples/pano-sample.jpg)
...
* CLEO/memory512.cs - увеличение лимитов для прорисовки
* CLEO/No_more_haze.cs - отключения эффекта жары
* CLEO/nohud.cs - отключение статы, радара и сообщений на экране

Желательно как-то настроить дистанцию отрисовки, к примеру через `0xBA6788` или специальные моды с заменой `timecyc.dat`
...
* Перед началом сессии выводить черный экран с надписями, чтобы точно синхронизировать начало записи
...
* Уменьшать громкость звуков на время скриншотов
* Оповещать при завершении скриншотов звуком
* Precache with dark screen
...
* Убрать ветер, зануление CWeather::Wind не работает
